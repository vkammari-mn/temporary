...
class SearchQuestions extends Component {
	...
	async displayQuestions(){
		const url = "http://mnipdrbhavanam:8888/lsforum/login/searchquestions?searchstring="+this.props.searchText
	...
	render(){
		return(
			...
			{this.state.display_qb && <div>{!this.state.answer_display && <input type="button" value="Post A Question" onClick={this.qb_act} class="btn-danger but-pos btn-lg" />}</div>}
		      {!this.state.answer_display && <div><h1 >Search Results-(Questions)</h1>
			<br />
			<ul>
